...
{
    public class AllSearchDecision : Decision
    {
        public AllSearchDecision(DroneAgent agent, GameState state) : base(agent, state)
        {
        ...
        {
            var fish = Agent.Fishes.OrderBy(_ => (_.Position - Agent.Drone.Position).LengthSqr()).First();
